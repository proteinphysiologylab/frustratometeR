...
print PML 
"load $ARGV[2]/VisualizationScrips/$ARGV[1].pdb
hide line,$ARGV[1]
...
for (my $i=0; $i<@AuxFile; $i++)
{
  ...
  if($splitted[4] eq "water-mediated")
  {
    ...
    {
      print PML "distance min_frst_wm_$ARGV[1]= ($ARGV[1]//$splitted[2]/$splitted[0]/CA),($ARGV[1]//$splitted[3]/$splitted[1]/CA)\n"; 
    }
    else
    {
      print PML "distance max_frst_wm_$ARGV[1]= ($ARGV[1]//$splitted[2]/$splitted[0]/CA),($ARGV[1]//$splitted[3]/$splitted[1]/CA)\n"; 
    }
    print JML "partial radius 0.1\n";
  }
  else
  {
    print PML "draw_links resi $splitted[0] and name CA and Chain $splitted[2] and $ARGV[1], resi $splitted[1] and name CA and Chain $splitted[3] and $ARGV[1], color=$splitted[5], color2=$splitted[5], radius=0.05, object_name=$splitted[0]:$splitted[1]_$splitted[5]_$ARGV[1]\n";
...
print PML "zoom all
hide labels
color red, max_frst_wm_$ARGV[1]
color green, min_frst_wm_$ARGV[1]";
